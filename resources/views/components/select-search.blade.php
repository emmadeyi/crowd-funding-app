...
>
    ...
    <div
        class="relative content-center w-full p-1 text-gray-500 font-light mt-1 text-left bg-white border rounded-md sm:text-sm sm:leading-5"
        x-bind:class="{'border-gray-300 ring ring-gray-200 ring-opacity-50':open, 'bg-gray-200  cursor-default':disabled}"
    ...
    >
        ...
        @isset($attributes['multiple'])
            <div class="flex flex-wrap space-x-1" x-cloak x-show="selected.length > 0">
                <template x-for="(key, index) in selected" :key="index">
                    <div class="text-gray-700 rounded-full truncate bg-gray-100 text-gray-500 px-2 py-0.5 my-0.5 flex flex-row items-center">
        ...
        >
            ...
            <div class="relative z-30 w-full p-2 bg-white">
                <input type="search" x-model="search" x-on:click.prevent.stop="open=true" class="block w-full p-2 border border-gray-300 rounded-md focus:border-gray-300 focus:ring focus:ring-gray-200 focus:ring-opacity-50 sm:text-sm sm:leading-5">
            ...
            <div x-ref="dropdown" class="relative z-30 p-2 overflow-y-auto max-h-60" >
                ...
                <template x-for="(key, index) in Object.keys(options)" :key="index" >
                    ...
                    <div
                        class="px-2 py-1"
                        x-bind:class="{'':selected.includes(key), 'hover:text-yellow-400 cursor-pointer':!(selected.includes(key)), 'text-yellow-400':currentIndex==index}"
                    ...
                    <div
                        class="px-2 py-1"
                        x-bind:class="{'':selected==key, 'hover:text-yellow-400 cursor-pointer':!(selected==key), 'text-yellow-400':currentIndex==index}"
                        @click.prevent.stop="selectOption(key)"
                        x-text="Object.values(options)[index]">
                    </div>
                    @endisset
