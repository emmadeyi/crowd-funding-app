<x-app-layout>
    ...
    @section('content-div')    
        ...
        <form class="image-upload" method="post" action="{{ route('careers.store') }}" >
            @csrf
            <div>
                <div class="col-span-6 sm:col-span-4 my-4">
                    <x-jet-label for="position" value="{{ __('Position') }}" />
                    <x-jet-input id="position" class="block mt-1 md:w-1/2" type="text" name="position" value="{{old('position')}}" required autofocus autocomplete="position" />
                    @error('position') <span class="error text-red-500">{{ $message }}</span> @enderror
                </div>
                <div class="col-span-6 sm:col-span-4 my-4">
                    <x-jet-label for="location" value="{{ __('Location') }}" />
                    <x-jet-input id="location" class="block mt-1 md:w-1/2" type="text" name="location" value="{{old('location')}}" required autofocus autocomplete="location" />
                ...
                <div class="col-span-6 sm:col-span-4 my-4">
                    ...
                    <x-jet-input id="salary" class="block mt-1 md:w-1/2" type="text" name="salary_range" value="{{old('salary_range')}}" autofocus autocomplete="salary_range" />
                    @error('salary_range') <span class="error text-red-500">{{ $message }}</span> @enderror
                </div>
                <div class="col-span-6 sm:col-span-4 my-4">
                    <x-jet-label for="close_date" value="{{ __('Close Date') }}" />
                    <x-jet-input id="close_date" class="block mt-1 md:w-1/2" type="date" name="close_date" value="{{old('close_date')}}" required autofocus autocomplete="close_date" />
                ...
                <div class="flex border-r border-gray-400 cursor-pointer p-2 w-auto mt-2 hover:bg-gray-100">
                    <x-jet-input id="publish" class="block mr-1 focus:border-gray-400 focus:ring focus:ring-gray-400 cursor-pointer" type="checkbox"  name="publish" />
                    <x-jet-label class="cursor-pointer" for="publish" value="Publish?" /> 
                </div>
                <div class="col-span-6 sm:col-span-4">
                    <x-jet-label for="editor" value="{{ __('Description') }}" />
                    <textarea id="editor" class="tinymce-editor block mt-1 w-1/2 rounded-md shadow-sm border-gray-300 focus:border-yellow-300 focus:ring focus:ring-yellow-200 focus:ring-opacity-50" name="description" required autofocus autocomplete="description"> {{old('description')}} </textarea> 
